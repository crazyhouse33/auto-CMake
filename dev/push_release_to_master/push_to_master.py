...
build_dir= "/opt/handCraftedUtilityShit/user_land_loader/build"
root_dir="/opt/handCraftedUtilityShit/user_land_loader"

change_dir(build_dir)
threatCommand("cmake --build . --target full-test")#configuring file and running memory tests
change_dir(root_dir)
threatCommand('git commit -a -m"Pushing release version  to master"')#commiting the readme and co change
